...
def col_filter_table(cols, table, header):
	...
	if indices==[]:
		return []
	return [[row[index] for index in indices] for row in table if len(row)>=(max(indices)+1)]
...
def main():
	parser = argparse.ArgumentParser(description="Run TraceBack workflow on AITQA.")
	...
	file_path = root_dir / 'Datasets' / 'AITQA' / 'aitqa_processed.jsonl'
	model_slug = sanitize_model_id(args.model)
	output_file_path = Path(args.output) if args.output else Path(f'aitqa_answer_attribution_{model_slug}_new.json')
	# Ensure custom output directories exist (e.g., --output results/aitqa/pred.json).
	...
	runner = TraceBackWorkflowRunner(
		...
		mysql_table_name=(args.mysql_table_name.strip() or f"traceback_tmp_aitqa_{os.getpid()}"),
	)
	processed = 0
	processed_ids = set()
	index_by_id = {}
	...
	log_file = log_dir / f'aitqa_{model_slug}_traceback.log'
	...
	pbar = tqdm(total=effective_total, desc=f"AITQA | {model_slug}", unit="ex", dynamic_ncols=True) if (tqdm and effective_total) else None
	...
	for line_no, data in _iter_jsonl(file_path):
		...
		try:
			...
			# Best-effort fallback: if workflow fails, fall back to empty attribution.
